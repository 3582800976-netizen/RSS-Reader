...
function SummaryBody({
  ...
  return (
    <div className="summary-md" dangerouslySetInnerHTML={{ __html: html }} />
  );
}
...
export default function ReaderPane({
  ...
  const [detailLevel, setDetailLevel] = useState<"concise" | "detailed">(
    "concise",
  );
  const [summaryLang, setSummaryLang] = useState<string>(() =>
    loadSummaryLanguage(),
  );
  const [summarizedLang, setSummarizedLang] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setSummarizedLang(null);
    if (!entry) return;
    api
      .getCachedSummary(entry.id)
      .then((s) => {
        if (s?.summary_text) {
          setSummaryText(s.summary_text);
          if (s.target_language) {
            setSummarizedLang(s.target_language);
            setSummaryLang(s.target_language);
          }
        }
      })
      .catch(() => undefined);
  ...
  function onSplitMouseDown(e: React.MouseEvent) {
    ...
    function onMove(ev: MouseEvent) {
      ...
      const newRatio = Math.min(
        0.85,
        Math.max(0.15, startRatio + dx / rect.width),
      );
  ...
  return (
    <section className="pane detail">
      ...
      {mode === "web" && entry.url ? (
        ...
        <div
          className={`dual ${dragging ? "dragging" : ""}`}
          ref={containerRef}
        >
          <article
            className="article"
          ...
          <div className="summary-panel">
            <div className="summary-toolbar">
              ...
              <select
                ...
                disabled={locked}
                onChange={(e) =>
                  setDetailLevel(e.target.value as "concise" | "detailed")
                }
              >
