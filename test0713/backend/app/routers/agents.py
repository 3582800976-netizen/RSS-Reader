...
from app.services.segmenter import html_to_plain, segment_html
    ...
    if not body.force:
        ...
        if cached:
            ...
            async def cached_gen():
                ...
                yield f"data: {json.dumps({'type': 'done', 'cached': True, 'model_name': cached['model_name']}, ensure_ascii=False)}\n\n"
    ...
    async def event_gen():
        ...
        try:
            ...
            if full:
                db.execute(
                    """
                    INSERT INTO entry_summaries (entry_id, summary_text, model_name, created_at)
                    VALUES (?, ?, ?, ?)
                    ...
                    """,
                    (body.entry_id, full, model_name, _utc_now()),
                )
                db.commit()
            yield f"data: {json.dumps({'type': 'done', 'cached': False, 'model_name': model_name}, ensure_ascii=False)}\n\n"
