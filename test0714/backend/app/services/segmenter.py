...
def segment_from_cleaned(cleaned_html: str | None) -> list[str]:
    """Split cleaned HTML into paragraph-level plain texts for translation.
    
    Uses the same segmentation logic as segment_html but consumes cleaned
    Readability output instead of raw Feed summary HTML.
    """
    if not cleaned_html or not cleaned_html.strip():
        return []
    return segment_html(cleaned_html)
