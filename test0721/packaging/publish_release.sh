...
DMG="$ROOT/release/Mercury Web-0721-arm64.dmg"
TAG="test0721-dmg"
...
NOTES="$(cat <<'EOF'
## Mercury Web — macOS 安装包（test0721）
...
[test/test0721](https://github.com/3582800976-netizen/RSS-Reader/tree/test/test0721)
EOF
)"
...
if gh release view "$TAG" --repo "$REPO" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" "$DMG" \
    --repo "$REPO" \
    --title "Mercury Web 0721 — macOS 安装包（Apple Silicon）" \
    --notes "$NOTES" \
    --latest
fi
