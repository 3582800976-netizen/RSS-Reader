...
from app.schemas import CleanedOut, ReadingPreferenceIn, ReadingPreferenceOut
from app.services.cleaning import clean_entry
from app.services.preferences import get_preferences, ReadingPreferences, save_preferences

router = APIRouter(prefix="/api", tags=["cleaning"])


@router.get("/entries/{entry_id}/cleaned", response_model=CleanedOut)
def get_cleaned(
    entry_id: int,
    ...
    try:
        result = clean_entry(conn, entry_id)
...
@router.get("/preferences/reading", response_model=ReadingPreferenceOut)
def get_reading_preferences(
    conn: sqlite3.Connection = Depends(get_connection),
) -> ReadingPreferenceOut:
...
@router.post("/preferences/reading", response_model=ReadingPreferenceOut)
def save_reading_preferences(
    body: ReadingPreferenceIn,
    conn: sqlite3.Connection = Depends(get_connection),
) -> ReadingPreferenceOut:
    current = get_preferences(conn)
    updated = ReadingPreferences(
        theme=body.theme if body.theme is not None else current.theme,
        font_size=body.font_size if body.font_size is not None else current.font_size,
        line_height=body.line_height if body.line_height is not None else current.line_height,
