...
export default function App() {
  ...
  const middleWidthRef = useRef(320);
  ...
  const feedsHidden = !isNarrow && (feedsCollapsed || readerFocus);
  const entriesHidden = !isNarrow && readerFocus;
    ...
    <div className="app">
      ...
      >
        <Sidebar
          ...
          feedUrl={feedUrl}
          busy={busy}
          collapsed={feedsHidden}
          style={{ width: feedsHidden ? 0 : leftWidth, flexShrink: 0 }}
        ...
        {!feedsHidden ? (
          <Resizer
            ...
            onResizeEnd={() =>
              api
                .saveReadingPrefs({ left_width: leftWidthRef.current })
                .catch(() => undefined)
            }
          />
        ) : null}
        ...
        <EntryList
          ...
          collapsed={entriesHidden}
          showFeedsToggle={!isNarrow}
          feedsCollapsed={feedsCollapsed}
          onToggleFeeds={() => setFeedsCollapsed((v) => !v)}
          onBack={
            isNarrow
              ? () => {
                  setMobilePane("feeds");
          ...
          style={{ width: entriesHidden ? 0 : middleWidth, flexShrink: 0 }}
          onSelect={(entry) => {
            clearPendingMarkRead();
        ...
        {!entriesHidden ? (
          <Resizer
            ...
            onResizeEnd={() =>
              api
                .saveReadingPrefs({ middle_width: middleWidthRef.current })
                .catch(() => undefined)
            }
          />
        ) : null}
        ...
        <ReaderPane
          ...
          busy={busy}
          isNarrow={isNarrow}
          showFocusToggle={!isNarrow}
          readerFocus={readerFocus}
          onToggleReaderFocus={() => setReaderFocus((v) => !v)}
          onBack={
            isNarrow
              ? () => {
                  clearPendingMarkRead();
