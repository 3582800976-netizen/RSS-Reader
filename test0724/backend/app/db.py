...
def _backfill_provider_model_names(conn: sqlite3.Connection) -> None:
    """Copy model_name from agent_settings onto providers that lack one."""
    rows = conn.execute(
        """
        SELECT p.id AS provider_id, a.model_name
        FROM llm_providers p
        JOIN agent_settings a ON a.provider_id = p.id
        WHERE IFNULL(TRIM(p.model_name), '') = ''
          AND IFNULL(TRIM(a.model_name), '') != ''
        ORDER BY a.agent_type ASC
        """
    ).fetchall()
    seen: set[int] = set()
    for row in rows:
        ...
        seen.add(pid)
        conn.execute(
            "UPDATE llm_providers SET model_name = ? WHERE id = ?",
            (row["model_name"], pid),
        )


def _migrate_summary_to_qa(conn: sqlite3.Connection) -> None:
    """One-time migration: copy summary agent settings to qa, then remove summary."""
    ...
    if not summary_row:
        return
    qa_row = conn.execute(
        "SELECT agent_type FROM agent_settings WHERE agent_type = 'qa'"
    ).fetchone()
    if not qa_row:
        conn.execute(
            """
            ...
            """,
            (
                summary_row["provider_id"],
                summary_row["model_name"],
                summary_row["target_language"],
                summary_row["detail_level"],
                summary_row["prompt_strategy"],
            ),
        )
    conn.execute("DELETE FROM agent_settings WHERE agent_type = 'summary'")
...
def init_db() -> None:
    ...
    try:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS entry_chat_messages (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                entry_id INTEGER NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS idx_entry_chat_messages_entry
                ON entry_chat_messages(entry_id);
        ...
        _ensure_column(conn, "llm_providers", "model_name", "model_name TEXT")
        _backfill_provider_model_names(conn)
        _migrate_summary_to_qa(conn)
